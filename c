...
def get_q1_growth(stock_code):
    """
    使用 baostock 获取季度营收和净利润同比增长率
    使用 query_growth_data 直接获取 YoY 增长率（小数形式，如 0.05=5%）

    返回 (revenue_growth, profit_growth, debug_info)
        - revenue_growth: 营收同比增长率（百分比，如 5.0 表示 5%）
        - profit_growth:  净利润同比增长率（百分比，如 5.0 表示 5%）
        - debug_info:     调试信息字符串
    """
    debug_info = []
    try:
        ...
        # 使用 query_growth_data 直接获取同比增长率
        # YoY 增长数据 (2026Q1 vs 2025Q1)
        rs = bs.query_growth_data(code=bs_code, year=2026, quarter=1)

        if rs.error_code != '0':
            msg = f"获取{stock_code}增长数据失败: {rs.error_msg}"
            debug_info.append(msg)
            log_debug(f"    {stock_code}: {msg}")
            bs.logout()
            return None, None, "\n".join(debug_info)

        if not rs.next():
            msg = f"未获取到{stock_code}的增长数据"
            debug_info.append(msg)
            log_debug(f"    {stock_code}: {msg}")
            bs.logout()
            return None, None, "\n".join(debug_info)
        ...
        row = rs.get_row_data()
        log_debug(f"    {stock_code} 2026Q1 growth_data: {row}")

        # growth_data 字段：
        # [0] code, [1] pubDate, [2] statDate
        # [3] yoRevenue        - 营业收入同比增长率（小数，如 0.0485 = 4.85%）
        # [4] yoNetProfit      - 净利润同比增长率（小数，如 0.0242 = 2.42%）
        if len(row) < 5:
            msg = f"增长数据字段不足: {row}"
            debug_info.append(msg)
            log_debug(f"    {stock_code}: {msg}")
        ...
        try:
            # 转为百分比（小数 * 100）
            revenue_raw = float(row[3]) if row[3] else 0.0
            profit_raw = float(row[4]) if row[4] else 0.0
        except (ValueError, TypeError) as e:
            msg = f"解析增长数据失败: {e}, row={row}"
            debug_info.append(msg)
            log_debug(f"    {stock_code}: {msg}")
            bs.logout()
            return None, None, "\n".join(debug_info)
        ...
        # 转换为百分比
        revenue_growth = round(revenue_raw * 100, 2)
        profit_growth = round(profit_raw * 100, 2)

        debug_info.append(f"获取到{stock_code} 2026Q1同比: 营收增长={revenue_growth:.2f}%, 净利增长={profit_growth:.2f}%")
