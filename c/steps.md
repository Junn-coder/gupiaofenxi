...
第 3 步（挑一只）：从 3 只里手动挑出 1 只 市值OK + 结构干净 + 尚未 4+ 连板 的票。
扫描器的 #1 通常是市值NG 的情绪温度计（如 600726），并"不是"最佳买点 ——
市值OK 的干净票通常排在 #2 或 #3。
...
第 5 步（仓位 + 止损）：按 framed.md §5A 双槽集中模式。
每槽 ≈ 25,000 元（账户的 50%），股数 = 25,000 / 入场价（向下取整到 100 股）。
实际风险由 ATR 止损宽度决定（framed.md §4A），而非固定百分比上限。
同时检查流动性：持仓金额不得超过该股 10 日平均成交额的 1%（§5A 流动性闸门）。
止损 = framed.md §4A 的 ATR 止损（max(5%, 1.5× 10日 ATR)，封顶 10%；
创业板/科创板用 1.0× ATR）或结构位下方 1-2% —— 取更接近入场价的那个。

第 6 步（试探入场）：先买目标股数的"一半"作为试探（framed §3 分批建仓）；
只有在下一个交易日确认（不弱）"且"闸门仍是绿灯时，才加另一半。不追高，不抄底。

第 7 步（记录 + 写次日计划）：用 "<code> <qty> <cost>" 更新 c/chold.md，并在次日开盘"前"
按 framed §6 写好管理计划 —— 止损触发（"收盘 < X => 次日开盘卖出"）、+8% 和 +15% 的止盈限价，
以及浮盈 > +10% 后启用的 "收盘 < 5 日均线" 移动止损。

第 8 步（机械管理）：每个收盘检查计划并在次日开盘动作 —— 收盘触及止损 = 全部卖出；
价格 >= +8%（约 入场价 × 1.08）= 减一半；价格 >= +15% = 减掉剩余；
炸板后回封失败 或 收盘 < 5 日均线 = 次日开盘出场。绝不补仓摊低成本；最多 10 个交易日后清仓（§4B 时间止损）。

第 9 步（复盘 + 重复）：收盘后重跑第 1 步并更新 c/watchlistd.md（最多 6 只）。
按 framed §6，规则违反"和"绿灯/暖市中错过的有效入场都要记录 —— 两者都算。

===
闸门 —— index.py → GREEN/AMBER/RED。
扫到 3 只 —— scan_cn.py --final 3。
挑一只 —— 市值OK 结构干净，不是扫描器的 #1 温度计。
验证历史 —— cn_stock.py <code> --history + framed §3 触发检查。
仓位 + 止损 —— 双槽集中（每槽 ~25K），ATR 止损（§4A），流动性检查（≤1% 日均成交额）。
试探入场 —— 半仓，只在次日确认后加。
记录 + 写计划 —— c/chold.md + 次日指令。
机械管理 —— 止损 / 止盈分批 / 5 日均线移动止损 / 绝不补仓。
...
读懂输出旗标（自动套用 framed.md）：
    市值✓  = 流通市值 50-500亿，framed.md §2 友好区间
    市值✗  = 太小（易控盘）或太大（拉不动）
    ⚠高位  = 连板 >= 5，高位接力风险，不追
...
python cn_stock.py 002927 --history --commit
